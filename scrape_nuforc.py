    ...
    for page_link_soup in nuforc_root_soup.find('table').find_all('a'):
        ...
        for report_link_soup in reports_by_date_soup.find('table').find_all('a'):
            try:
                ...
                if (report_soup == None):
                    print(" -- Failed at empty page: " + link_to_report_page)
                    continue

                # Parse report metadata
                report_metadata_soup = report_soup.find('td')
                if (report_metadata_soup == None):
                    print(" -- Failed at page missing metadata: " + link_to_report_page)
                    continue
                ...
                # Runs slower with this included
                # print('.', end='', flush=True)

            # Be lenient so that aberrant pages don't kill the script
            except Exception as err:
                print("-- Fatal error with this report")
